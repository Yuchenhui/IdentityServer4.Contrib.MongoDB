    ...
    public class ConsentService
    {
        ...
        public async Task<ProcessConsentResult> ProcessConsent(ConsentInputModel model)
        {
            ...
            else if (model.Button == "yes")
            {
                ...
                if (model.ScopesConsented != null && model.ScopesConsented.Any())
                {
                    ...
                    {
                        RememberConsent = model.RememberConsent, ScopesValuesConsented = scopes.ToArray()
                    };
                }
                else
                {
        ...
        public async Task<ConsentViewModel> BuildViewModelAsync(string returnUrl, ConsentInputModel model = null)
        {
            ...
            if (request != null)
            {
                ...
                {
                    Resources resources =
                        await _resourceStore.FindEnabledResourcesByScopeAsync(request.ValidatedResources.Resources
                            .ToScopeNames());
                    ...
                    _logger.LogError("No scopes matching: {0}",
                        request.ValidatedResources.Resources.ToScopeNames().Aggregate((x, y) => x + ", " + y));
                }
                else
                {
            ...
            Client client, Resources resources)
        {
            ...
            vm.ResourceScopes = resources.ApiScopes
                .Select(x => CreateScopeViewModel(x, vm.ScopesConsented.Contains(x.Name) || model == null)).ToArray();
